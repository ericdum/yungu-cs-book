...
createApp({
    data() {
        return {
            ...
            nextSection: null // 下一个要阅读的 section
        };
    },
    async mounted() {
        // 初始化字体大小
    ...
    methods: {
        ...
        /**
         * 计算并更新下一个要阅读的 section
         */
        updateNextSection() {
            if (!this.navigation || !this.currentPath) {
                this.nextSection = null;
                return;
            }
            
            // 找到当前 section 在导航中的位置
            ...
            for (let i = 0; i < this.navigation.length; i++) {
                ...
                // 检查是否是 page 类型
                if (item.type === 'page' && item.path === this.currentPath) {
                    isPageType = true;
            ...
            // 如果是 page 类型，找下一个 item（跳过其他 page，找第一个 chapter 的第一个 section）
            if (isPageType) {
                // 从下一个 item 开始查找
                for (let i = currentChapterIndex + 1; i < this.navigation.length; i++) {
                    const item = this.navigation[i];
                    if (item.type === 'chapter' && item.sections && item.sections.length > 0) {
                        this.nextSection = {
                            path: item.sections[0].path,
            ...
            // 如果是 chapter 类型中的 section
            const currentChapter = this.navigation[currentChapterIndex];
            
            // 检查当前 section 是否是所在章节的最后一个
            if (currentSectionIndex < currentChapter.sections.length - 1) {
                // 不是最后一个，返回下一个 section
            ...
            // 是最后一个 section，找下一个章节的第一个 section
            for (let i = currentChapterIndex + 1; i < this.navigation.length; i++) {
                const item = this.navigation[i];
                if (item.type === 'chapter' && item.sections && item.sections.length > 0) {
                    this.nextSection = {
        ...
        goToNextSection() {
            if (this.nextSection) {
                this.loadSection(this.nextSection.path, this.nextSection.title);
            }
        }
    }
}).mount('#app');
